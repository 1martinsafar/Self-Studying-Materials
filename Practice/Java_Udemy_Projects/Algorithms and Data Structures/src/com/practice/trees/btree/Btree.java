...
public class Btree {
    ...
    private Node Search(int x, Node p) {
        if (p == null) { // Key x not present in the tree, reached a leaf node
            return null;
        }
    ...
    public void Insertion(int x) {
        ...
        if (taller) { // Height increased by 1, new root node has to be created
    ...
    private void Restore(Node p, int n) {
        if (n != 0 && p.child[n-1].numKeys > MIN) { // Check left sibling
            BorrowLeft(p, n);
        }
        else if (n != p.numKeys && p.child[n+1].numKeys > MIN) { // Check right sibling
            BorrowRight(p, n);
        }
        else {
    ...
    private void BorrowLeft(Node p, int n) {
        ...
        // Shift all the keys and children in underflowNode 1 position right
        ...
        // Rightmost child of leftSiblingbecomes leftmost child of underflowNode
    ...
    private void BorrowRight(Node p, int n) {
        ...
        // Shift all the keys and children of rightSibling 1 position left
    ...
    private void Combine(Node p, int m) {
        ...
        // Shift the keys and children that are after separator key in node p 1 position left
        int i;
